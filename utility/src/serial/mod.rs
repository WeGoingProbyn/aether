...
use crate::error::{AetherError, AetherResult, ErrorDomain, UtilityErrorKind};
...
impl<R: std::io::Read> TextReader<R> {
  ...
  pub(crate) fn expect_byte(&mut self, expected: u8) -> AetherResult<()> {
    match self.read_byte()? {
      ...
      Some(b) => Err(
        AetherError::new(ErrorDomain::Utility(UtilityErrorKind::UnexpectedByte))
        .context(&format!("a text reader expected byte: {} but found: {}", expected, b))
      ),
      None => Err(
        AetherError::new(ErrorDomain::Utility(UtilityErrorKind::UnexpectedEof))
