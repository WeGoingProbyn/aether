...
\begin{itemize}
  ...
  \item \textbf{Topology}: represents \emph{connectivity} between discrete entities (neighbors, boundaries, patch seams).
...
The central design goal is to keep the finite-volume solver largely independent of the physical coordinate system or embedding: if the solver can query \emph{neighbor relations} (Topology) and \emph{geometric measures} (Geometry), it can advance conservative PDEs on many kinds of domains (Cartesian, curvilinear, multi-block, cubed-sphere, etc.).
...
\begin{itemize}
  ...
  \item \textbf{Neighbor info} for each face: which cell or boundary is adjacent to the face
\end{itemize}
...
\begin{itemize}
  ...
  \item For each face, identifies the neighbor cell across the face, or a boundary condition object (wall, inflow, outflow, periodic, etc.).
...
\begin{enumerate}
  ...
  \item \textbf{Topology}: for each cell $i$, iterate its faces and obtain neighbor/boundary information.
