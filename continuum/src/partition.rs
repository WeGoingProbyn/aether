...
impl<const D: usize, M: CellGeometry<D> + FaceGeometry<D> + Topology> Decomposition<D, M> {
  /// Exchange ghost cell data between partition fields.
  /// Two-pass to avoid aliasing: collect all ghost values, then scatter.
  #[profile]
  pub fn exchange_ghosts<const N: usize, S: FieldStorage<N>>(&self, fields: &mut [S]) {
    // Pass 1: collect ghost values from source partitions
    let ghost_data: Vec<Vec<(CellId, [f64; N])>> = self.partitions.iter()
      .map(|p| {
        p.ghost_cells.iter().map(|g| {
          let val = *fields[g.source_partition]
            .state(g.source_local_cell).as_state();
          (g.local_cell, val)
        }).collect()
      }).collect();
...
pub fn decompose_structured<const D: usize>(
  ...
  let partitions = (0..num_parts).map(|p| {
    ...
    let mut boundary_map: HashMap<BoundaryTag, Vec<(FaceId, CellId)>> = HashMap::new();
    ...
    for axis in 0..D {
      ...
      for local_in_axis in 0..face_count {
        let face_ijk = StructuredBlock::<D>::face_indices(&dims, axis, local_in_axis);
        let gf = FaceId::from(global_face);

        // Determine which cells this face connects
        let conn = if face_ijk[axis] == 0 {
          // Min boundary
          let cell_global = CellId::from(
            StructuredBlock::<D>::cell_index(&dims, &face_ijk)
          );
          FaceKind::Boundary {
          ...
          let cell_global = CellId::from(
            StructuredBlock::<D>::cell_index(&dims, &cell_ijk)
          );
          FaceKind::Boundary {
            owner_global: cell_global,
          ...
          owner_ijk[axis] -= 1;
          let owner_global = CellId::from(
            StructuredBlock::<D>::cell_index(&dims, &owner_ijk)
          );
          let neighbour_global = CellId::from(
            StructuredBlock::<D>::cell_index(&dims, &face_ijk)
          );
          FaceKind::Interior {
            owner_global,
        ...
        match conn {
          FaceKind::Interior { owner_global, neighbour_global } => {
            let owner_local = global_to_local_cell.get(&owner_global).copied();
            let nbr_local = global_to_local_cell.get(&neighbour_global).copied();
          ...
          FaceKind::Boundary { owner_global, tag, out_sign } => {
...
fn build_ijk<const D: usize>(
  dims: &[usize; D], split_axis: usize, sa_pos: usize, cross_flat: usize,
) -> [usize; D] {
  let mut ijk = [0; D];
  ijk[split_axis] = sa_pos;
  let mut remaining = cross_flat;
  for d in 0..D {
    if d == split_axis { continue; }
...
enum FaceKind {
  Interior { owner_global: CellId, neighbour_global: CellId },
  Boundary { owner_global: CellId, tag: BoundaryTag, out_sign: f64 },
}
